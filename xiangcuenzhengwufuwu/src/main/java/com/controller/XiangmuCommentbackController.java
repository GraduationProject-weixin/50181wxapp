...
public class XiangmuCommentbackController {
    ...
    */
    ...
    public R update(@RequestBody XiangmuCommentbackEntity xiangmuCommentback, HttpServletRequest request){
...
//            xiangmuCommentback.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<XiangmuCommentbackEntity> queryWrapper = new EntityWrapper<XiangmuCommentbackEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        XiangmuCommentbackEntity xiangmuCommentbackEntity = xiangmuCommentbackService.selectOne(queryWrapper);
        xiangmuCommentback.setUpdateTime(new Date());
        if(xiangmuCommentbackEntity==null){
            //  String role = String.valueOf(request.getSession().getAttribute("role"));
            //  if("".equals(role)){
            //      xiangmuCommentback.set
            //  }
...
//                            xiangmuCommentbackEntity.setXiangmuId(Integer.valueOf(data.get(0)));   //预约项目 要改的
...
//                            xiangmuCommentbackEntity.setUpdateTime(new Date(data.get(0)));          //回复时间 要改的
//                            xiangmuCommentbackEntity.setCreateTime(date);//时间
    ...
    */
    @RequestMapping("/add")
    public R add(@RequestBody XiangmuCommentbackEntity xiangmuCommentback, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,xiangmuCommentback:{}",this.getClass().getName(),xiangmuCommentback.toString());
        xiangmuCommentback.setInsertTime(new Date());
        xiangmuCommentback.setCreateTime(new Date());
        xiangmuCommentbackService.insert(xiangmuCommentback);
        return R.ok();
